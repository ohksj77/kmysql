...
class RecoveryManager(
    ...
    private fun doRollback() {
        ...
        while (iterator.hasNext()) {
            ...
            if (record != null && record.transactionNumber() == transactionNumber) {
    ...
    private fun doRecover() {
        ...
        while (iterator.hasNext()) {
            ...
            if (record.op() == Operator.COMMIT.id || record.op() == Operator.ROLLBACK.id) {
                finishedTransaction.add(record.transactionNumber())
            } else if (!finishedTransaction.contains(record.transactionNumber())) {
                record.undo(transaction)
            }
        }
    }
